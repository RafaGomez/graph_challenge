...
export class GraphService {
  ...
  public validateGraph (graph: Graph): String[] {
    ...
    if (errors.length == 0)
    {
      errors.concat(this.validateLinks(graph));
    }
  ...
  private validateNodeTypes(graph: Graph): String[]{
    ...
    while (index < graph.nodes.length) {      
      startNodes = (graph.nodes[index].type === NodeTypeEnum.Init) ? startNodes + 1 : startNodes;
      endNodes += (graph.nodes[index].type === NodeTypeEnum.End) ? 1 : 0;
      actionFound = actionFound || (graph.nodes[index].type === NodeTypeEnum.Action);
      conditionFound = conditionFound || (graph.nodes[index].type === NodeTypeEnum.Condition); 
      index++;
    }
    if (!(actionFound && conditionFound && (startNodes > 0) && (endNodes > 0)))
    {
      errors.push("A Workflow should have at least a Init, End, Condition an Action node.");
    }
    if (startNodes > 1){
          ...
          switch(node.type){
            ...
            default: {
              if (outboundLinks == 0 || inboundLinks == 0){
                errors.push("${node.description} node must have at least one inbound and outbound link.");
              }
            }
          }
        });
